...
`scripts/carregar_recorte_ci.py` (mesma lógica de `app/services/
cnpj_recorte_service.py`), disparado a cada 30 min por um runner do
GitHub Actions — nunca pelo Render (raio-X 2026-09-09: ver DEPLOY.md
seção 7.1) — baixando os arquivos direto da Receita Federal, sem passo
humano. Este script continua existindo só pra depurar localmente sem
depender do cron, ou como plano B se o download automático falhar.
