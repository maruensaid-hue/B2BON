...
def test_criar_tenant_vitrine_colisao_de_slug_gera_sufixo(db_session):
    ...
    usuario1, _ = tenant_service.criar_tenant_vitrine(
        db_session, convite1.codigo, "Mesma Empresa", "Admin 1", "admin3@convidada.com.br", "senha123", True, plano.id, StubPaymentProvider()
    )
    usuario2, _ = tenant_service.criar_tenant_vitrine(
        db_session, convite2.codigo, "Mesma Empresa", "Admin 2", "admin4@convidada.com.br", "senha123", True, plano.id, StubPaymentProvider()
    )
...
def test_convite_vitrine_usado_duas_vezes_falha(db_session):
    ...
    plano = _plano(db_session)
    tenant_service.criar_tenant_vitrine(
        db_session, convite.codigo, "Empresa A", "Admin A", "a@convidada.com.br", "senha123", True, plano.id, StubPaymentProvider()
    )

    with pytest.raises(RegraNegocioViolada):
        tenant_service.criar_tenant_vitrine(
            db_session, convite.codigo, "Empresa B", "Admin B", "b@convidada.com.br", "senha123", True, plano.id, StubPaymentProvider()
        )
...
def test_convite_vitrine_revogado_bloqueia_aceite(db_session):
    ...
    with pytest.raises(RegraNegocioViolada):
        tenant_service.criar_tenant_vitrine(
            db_session, convite.codigo, "Empresa C", "Admin C", "c@convidada.com.br", "senha123", True, plano.id, StubPaymentProvider()
        )
...
def test_convite_vitrine_expirado_bloqueia_aceite(db_session):
    ...
    with pytest.raises(RegraNegocioViolada):
        tenant_service.criar_tenant_vitrine(
            db_session, "VITRINEEXPIRADO", "Empresa D", "Admin D", "d@convidada.com.br", "senha123", True, plano.id, StubPaymentProvider()
        )
...
def test_convite_vitrine_inexistente_levanta_nao_encontrado(db_session):
    ...
    with pytest.raises(NaoEncontrado):
        tenant_service.criar_tenant_vitrine(
            db_session, "CODIGO-QUE-NAO-EXISTE", "Empresa E", "Admin E", "e@convidada.com.br", "senha123", True, plano.id, StubPaymentProvider()
        )
...
def test_criar_tenant_vitrine_email_ja_cadastrado_falha(db_session):
    ...
    plano = _plano(db_session)
    tenant_service.criar_tenant_vitrine(
        db_session, convite1.codigo, "Empresa F", "Admin F", "repetido@convidada.com.br", "senha123", True, plano.id, StubPaymentProvider()
    )

    convite2 = tenant_service.gerar_convite_vitrine(db_session, TENANT_ID_ORIGEM, None, validade_horas=24)
    with pytest.raises(RegraNegocioViolada):
        tenant_service.criar_tenant_vitrine(
            db_session, convite2.codigo, "Empresa G", "Admin G", "repetido@convidada.com.br", "senha123", True, plano.id, StubPaymentProvider()
        )
