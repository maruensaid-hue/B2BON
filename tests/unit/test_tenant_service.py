...
def _representante(db_session) -> Representante:
    numero = db_session.query(Representante).count() + 1
    representante = Representante(
        nome=f"Representante {numero}", email=f"representante{numero}@teste.com.br",
        chave_pix=f"pix-{numero}@teste.com.br", percentual_comissao=0.1,
    )
...
def test_criar_tenant_publico_gera_licenca_pendente_de_pagamento(db_session):
    ...
    usuario, checkout_url = tenant_service.criar_tenant_publico(
        ...
        StubPaymentProvider(), representante.id,
    )
    ...
    assert tenant.representante_id == representante.id
    licenca = db_session.query(Licenca).filter_by(tenant_id=usuario.tenant_id).one()
    assert licenca.status == "pendente_pagamento"
    assert licenca.plano_id == plano.id
...
def test_criar_tenant_publico_representante_inativo_e_recusado(db_session):
    plano = _plano(db_session)
    representante = _representante(db_session)
    ...
    with pytest.raises(RegraNegocioViolada):
        tenant_service.criar_tenant_publico(
            db_session, "Empresa Rep Inativo", "Admin", "rep-inativo@publica.com.br", "senha123", True, plano.id,
            StubPaymentProvider(), representante.id,
        )
