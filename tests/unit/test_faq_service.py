...
def test_responder_com_historico_inclui_turnos_anteriores_no_prompt():
...
def test_responder_trunca_historico_nos_ultimos_seis_turnos():
    llm = FakeLLMProvider(["resposta"])
    historico = [("usuario" if i % 2 == 0 else "ia", f"turno {i}") for i in range(10)]

    faq_service.responder("pergunta nova", llm, historico=historico)
...
def test_responder_sem_historico_mantem_prompt_igual_a_pergunta():
    llm = FakeLLMProvider(["resposta"])

    faq_service.responder("pergunta isolada", llm, historico=None)
