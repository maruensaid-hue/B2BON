...
def test_importar_participantes_cria_contas_e_decisores(client, criar_icp):
    icp = criar_icp()
...
def test_importar_participantes_enfileira_so_contas_novas_para_enriquecimento(client, criar_icp):
    """Pedido do usuário: planilha importada não fica dependendo de
    enriquecer conta por conta na mão — as novas entram numa fila
    processada em lote pelo cron. Empresa já existente (reaproveitada)
    não entra de novo — presumivelmente já foi enriquecida antes."""
    icp = criar_icp()
