...
_DESCRICAO_14133 = "Contratação pública brasileira (Lei 14.133/2021). Os parâmetros são padrões analíticos do B2B ON, não limites legais."
_FONTE_14133 = "Lei nº 14.133, de 1º de abril de 2021 (Lei de Licitações e Contratos Administrativos)"
...
# @1 fica registrado para o histórico (processos e espelhos antigos citam o código).
LEI_14133_V1 = ruleset.registrar(ruleset.Ruleset(
    codigo="PUBLIC_PROCUREMENT_BR_14133@1", descricao=_DESCRICAO_14133, fonte=_FONTE_14133, vigente_desde=date(2021, 4, 1),
    documentos_esperados=_DOCUMENTOS_14133, parametros=_PARAMETROS_14133_V1,
))
# @2 (Phase J2, TD-089): os limiares analíticos de risco saem do código e passam a ser parâmetros, com os mesmos
# padrões de antes e configuráveis pelo órgão (`orgao.parametros`). Mudança de parâmetro = nova versão.
LEI_14133 = ruleset.registrar(ruleset.Ruleset(
    codigo="PUBLIC_PROCUREMENT_BR_14133@2", descricao=_DESCRICAO_14133, fonte=_FONTE_14133,
    vigente_desde=date(2021, 4, 1),  # art. 194: vigor na data da publicação
    documentos_esperados=_DOCUMENTOS_14133,
    parametros={
        **_PARAMETROS_14133_V1,
        "aditivos_alerta": 3,          # contrato com N aditivos ou mais = sinal para revisão
        "acrescimo_alerta": 0.25,      # acréscimo acima de 25% do valor inicial (art. 125: limite usual de acréscimos)
        "concentracao_alerta": 0.5,    # um fornecedor com mais de 50% do valor contratado numa categoria
        "dias_planejamento": 60,       # item do PCA previsto em até N dias sem processo ativo
    },
))
