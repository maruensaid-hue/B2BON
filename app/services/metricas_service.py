...
def calcular_roi(ltv_medio: float | None, cac: float | None) -> float | None:
    """Delegado ao MAP (`app/contexts/map/economics.py`) desde a Fase 1."""
    return map_contract.calcular_roi(ltv_medio, cac)


def calcular_cs_score(db: Session, tenant_id: str, conta_ids: list[int], scores_risco: list[float]) -> dict:
    """Delegado ao MAP (`app/contexts/map/economics.py`) desde a Fase 1."""
    return map_contract.cs_score(db, tenant_id, conta_ids, scores_risco)
