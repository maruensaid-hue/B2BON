    ...
    resposta = intel.gerar(
        ...
        LLMRequest(
            prompt=(
                ...
                f"{intel.prompt_seguro.bloco_dados_externos('atividades_registradas', linhas_atividades)}"
                f"{secao_padroes}\n\n"
...
# --- Aliases (Fase 1, Strangler) ----------------------------------------------
# Prospecção foi para `app/contexts/predator/prospeccao.py` e os helpers de
# Organization para o Shared Kernel. Os nomes abaixo mantêm os chamadores
# antigos funcionando; novos chamadores devem importar do contexto.
obter = organizations.obter_conta
decisores_da_conta = organizations.decisores_da_conta
_normalizar_dominio = organizations.normalizar_dominio
gerar_lista = predator_contract.gerar_lista
enfileirar_enriquecimento_em_lote = predator_contract.enfileirar_enriquecimento_em_lote
enriquecer = predator_contract.enriquecer
enriquecer_via_brasilapi = predator_contract.enriquecer_via_brasilapi
mapear_decisores = predator_contract.mapear_decisores
