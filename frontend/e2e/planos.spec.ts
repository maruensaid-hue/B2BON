...
test("página de planos mostra o catálogo sem vender o que não foi lançado", async ({ page }) => {
  ...
  const procurement = catalogo.locator("div.rounded-xl", { hasText: "Public Procurement" });
  await expect(procurement.getByText("Em breve · preço em definição")).toBeVisible();
  await expect(procurement.getByRole("link", { name: /Assin/ })).toHaveCount(0);
  await expect(procurement.getByText(/R\$/)).toHaveCount(0);

  await expect(catalogo.locator("div.rounded-xl", { hasText: "Bid Intelligence" }).getByText("Sob consulta")).toBeVisible();
