...
| TD-039 | Shims de compatibilidade em `conta_service`, `crm_service` e `metricas_service` (aliases para os contextos) | Fase 1 | Dois caminhos para a mesma função | 2–3 (remover quando não houver chamador) | OPEN (Phase J2 avaliou: os aliases ainda têm muitos chamadores — remover exige mexer em vários contextos sem ganho de comportamento; fica para quando os chamadores migrarem) |
| TD-040 | `decisores_da_conta(db, conta_id)` não filtra por tenant (confia em validação prévia do chamador) | `contexts/shared/organizations.py` | Risco de isolamento se chamado sem `obter_conta` antes | 2 | RESOLVED (Phase J2: `decisores_da_conta` recebe a conta já validada e filtra também pelo tenant dela; teste de isolamento) |
...
| TD-081 | Sourcing duplicado entre `bids` e `procurement`: 2 tabelas de documento, 2 pipelines de extração, requisito em tabela × JSON (`achados`), 2 contratos, 2 workspaces (backend e UI) | `18_STRATEGIC_SOURCING.md` §1.2 (D1–D6) | Toda melhoria feita duas vezes; Enterprise dobraria | S1–S6 | IN_PROGRESS (S1–S3): engines, leitura por lado e **tabelas unificadas** (espelhadas e conferidas) prontos; troca de leitura e remoção das tabelas antigas na S6; workspaces na S5 |
...
| TD-085 | Quatro implementações de score de aderência (ICP, oferta, intenção/rede, fornecedor) sem base comum | `18_STRATEGIC_SOURCING.md` D7 | Explicações e UNKNOWN inconsistentes | S1 (por toque) | IN_PROGRESS (S1): base `shared/matching.py`; ICP migrado (PREDATOR e rede, com paridade); oferta, intenção e fornecedor migram por toque |
...
| TD-087 | Até a S6 toda escrita de sourcing grava duas vezes (tabela antiga + espelho) e as leituras dos repositórios conferem as tabelas novas (COMPARAR) | `bids/espelho.py`, `procurement/espelho.py`, repositórios | Custo extra de escrita/leitura durante a migração; some na S6 | S6 | OPEN (temporária, por desenho). Phase J1 (D-070): leitura de listagem pelas tabelas novas pronta atrás de `sourcing_leitura_fonte` (desligada); falta mover `obter_processo`/escrita e o comprador |
| TD-088 | Arquivo e texto das páginas continuam só nas tabelas de origem; `documento_sourcing` guarda só metadados | `app/models/sourcing.py` | S6 precisa mover os blobs (tabela 1:1 ou armazenamento de objetos) antes de remover as tabelas antigas | S6 | OPEN |
| TD-089 | Limiares analíticos de risco do comprador (`DIAS_PLANEJAMENTO`, `ACRESCIMO_ALERTA`, `ADITIVOS_ALERTA`, `CONCENTRACAO_ALERTA`) ainda são constantes em `riscos.py`, fora do ruleset | `procurement/riscos.py` | Não configuráveis por órgão; o de acréscimo (25%) é regra da Lei 14.133 e deveria estar no ruleset | Próxima versão do ruleset (`PUBLIC_PROCUREMENT_BR_14133@2`) | RESOLVED (Phase J2: limiares no ruleset `PUBLIC_PROCUREMENT_BR_14133@2`, mesmos padrões, configuráveis pelo órgão nos de contrato; @1 segue registrado) |
| TD-090 | Sinais de risco do comprador consultam a pesquisa de preços uma vez por processo aberto (`precos.resumo` dentro do laço) | `procurement/riscos.py:79` | N+1: 40 processos → 49 consultas em `/procurement/riscos` e 58 no workspace (baseline da Phase A) | Phase D | RESOLVED (Phase D, D-064) |
| TD-091 | Semente preguiçosa dos catálogos (`/catalogo`, `/ai-credits/workloads`) disputa escrita quando a página pública dispara as chamadas em paralelo no primeiro acesso; no SQLite (dev/E2E) uma delas responde 500 `database is locked` | `ai_credits`, catálogo comercial | Só SQLite (Postgres tem SAVEPOINT + releitura, testado na Fase 15); página de planos pode abrir sem um bloco no primeiro acesso em dev | Semear na subida da aplicação (lifespan) ou tornar a leitura sem escrita | RESOLVED (Phase H, D-068: semente na subida + no seed do E2E; E2E com 0 `database is locked`) |
| TD-092 | Conectores Pipedrive e RD Station repetem 12 linhas que agrupam atividades em interações por conta | `integrations/adapters/pipedrive.py`, `rd_station.py` | Pouco: conectores BETA e desligados (TD-069) | Extrair quando um conector for ativado ou mudar essa regra | RESOLVED (Phase J2: `adapters/interacoes.py` compartilhado pelos dois conectores) |
