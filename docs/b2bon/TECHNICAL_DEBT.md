...
| TD-089 | Limiares analíticos de risco do comprador (`DIAS_PLANEJAMENTO`, `ACRESCIMO_ALERTA`, `ADITIVOS_ALERTA`, `CONCENTRACAO_ALERTA`) ainda são constantes em `riscos.py`, fora do ruleset | `procurement/riscos.py` | Não configuráveis por órgão; o de acréscimo (25%) é regra da Lei 14.133 e deveria estar no ruleset | Próxima versão do ruleset (`PUBLIC_PROCUREMENT_BR_14133@2`) | OPEN |
...
| TD-092 | Conectores Pipedrive e RD Station repetem 12 linhas que agrupam atividades em interações por conta | `integrations/adapters/pipedrive.py`, `rd_station.py` | Pouco: conectores BETA e desligados (TD-069) | Extrair quando um conector for ativado ou mudar essa regra | OPEN |
