...
| TD-082 | Consultas de documento carregam o arquivo binário e o texto de todas as páginas (sem `defer`); sinais de risco fazem N+1 com esses blobs | `bids/workspace.py`, `conformidade.py`, `grafo.py`, `procurement/workspace.py`, `procurement/riscos.py:71` | Latência e memória crescem com o tamanho dos editais | S0 | OPEN |
...
| TD-084 | Listas de licitações e de processos sem paginação | `bids/licitacoes.py`, `procurement/cadastros.py` | Resposta cresce sem limite | S0 | OPEN |
