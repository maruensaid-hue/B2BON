...
| TD-081 | Sourcing duplicado entre `bids` e `procurement`: 2 tabelas de documento, 2 pipelines de extração, requisito em tabela × JSON (`achados`), 2 contratos, 2 workspaces (backend e UI) | `18_STRATEGIC_SOURCING.md` §1.2 (D1–D6) | Toda melhoria feita duas vezes; Enterprise dobraria | S1–S6 | IN_PROGRESS (S1): extração, preparação de documento e avaliação já são um engine só (`contexts/sourcing`); tabelas, contratos e workspaces unificam na S3–S5 |
