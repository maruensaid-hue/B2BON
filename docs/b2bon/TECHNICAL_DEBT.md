...
| TD-081 | Sourcing duplicado entre `bids` e `procurement`: 2 tabelas de documento, 2 pipelines de extração, requisito em tabela × JSON (`achados`), 2 contratos, 2 workspaces (backend e UI) | `18_STRATEGIC_SOURCING.md` §1.2 (D1–D6) | Toda melhoria feita duas vezes; Enterprise dobraria | S1–S6 | IN_PROGRESS (S1–S2): extração, preparação de documento, avaliação e leitura por lado já são compartilhadas (`contexts/sourcing`, repositórios); tabelas, contratos e workspaces unificam na S3–S5 |
