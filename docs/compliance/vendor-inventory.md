...
| **Neon** | Banco relacional principal (Postgres gerenciado) | Praticamente todo dado pessoal da plataforma | EUA (empresa), região do banco a confirmar | Real, infraestrutura core | Não — DPA da Neon já existe publicamente (https://neon.com/pdf/DPA.pdf), falta revisão formal |
| **Render** | Hospedagem do backend | Dado em trânsito/memória/logs técnicos | EUA (empresa), região do serviço a confirmar | Real, infraestrutura core | Não — DPA da Render já existe publicamente (https://render.com/dpa), falta revisão formal |
| **Neo4j** (AuraDB Free) | Grafo de relacionamento (espelho de visualização) | `tenant_id`, nome/CNPJ de conta, nome/cargo de decisor — nunca e-mail/telefone | A confirmar | Real | Não |
